...
namespace MvcPersona.Controllers
{
    public class PersonasController : Controller
    {
        ...
        {
            var NombreLst = new List<string>();

            var NombreQry = from d in db.Personas
                            orderby d.Nombre
                            select d.Nombre;

            NombreLst.AddRange(NombreQry.Distinct());
            ViewBag.buscarNombre = new SelectList(NombreLst);
            ...
            {
                personas = personas.Where(s => s.DUI.Contains(buscarDUI));
            }

            if (!String.IsNullOrEmpty(buscarNombre))
            {
                personas = personas.Where(x => x.Nombre == buscarNombre);
            }
